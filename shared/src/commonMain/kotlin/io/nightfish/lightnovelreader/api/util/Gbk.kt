 ...
 * GBK/GB18030 编码工具。
 *
 * wenku8 页面是 GB18030 编码，Kotlin/Native 的 kotlin.text 没有内置 GB18030 charset，
 * 所以这里手写两张表：
 * - 编码表：把常见汉字/符号编码成 GBK 双字节（用于搜索关键词百分号编码）
 * - 解码表：把 GBK/GB18030 双字节序列还原成 Unicode（用于解析 HTML 正文）
 *
 * 覆盖范围：
 * - ASCII 原样
 * - GB2312 一级(3755) + 二级(3008) 汉字 = 6763 个常用汉字
 * - 全角标点/日文假名等常见 GBK 扩展区字符
 * 生僻字（GBK 未覆盖）以 `?` 占位，wenku8 搜索关键词场景影响可忽略。
 */
object Gbk {

    // MARK: - 编码（Unicode → GBK 双字节）

    private val encoderMap: Map<Char, Int> = buildEncoderMap()

    /** 把字符串编码成 GBK 字节序列；无法编码的字符用 0x3F('?') 替代 */
    fun encodeToBytes(text: String): ByteArray {
        val out = ArrayList<Byte>(text.length * 2)
        for (ch in text) {
            when {
                ch.code < 0x80 -> out.add(ch.code.toByte())
                else -> {
                    val gbk = encoderMap[ch]
                    if (gbk != null) {
                        out.add((gbk shr 8).toByte())
                        out.add((gbk and 0xFF).toByte())
                    } else {
                        out.add(0x3F) // '?'
                    }
                }
    ...
    /** 百分号编码（URLEncoder.encode(keyword, "gb2312") 的 KMP 替代）：非 ASCII 字节 → %XX */
    fun percentEncode(text: String): String {
        val bytes = encodeToBytes(text)
    ...
    // MARK: - 解码（GBK/GB18030 双字节 → Unicode）

    /** GBK 双字节区段（GB2312 6763 汉字 + GBK 扩展） */
    private val decoderMap: Map<Int, Char> = buildDecoderMap()
    ...
    fun decodeToString(bytes: ByteArray): String {
        ...
        while (i < bytes.size) {
            ...
            if (i + 1 < bytes.size) {
                ...
                // GBK 尾字节合法范围：0x40-0xFE（除 0x7F）。第二字节不合法时按单字节处理，避免中文+ASCII 混合错位
                if (second >= 0x40 && second != 0x7F) {
                    val key = (first shl 8) or second
                    val decoded = decoderMap[key]
                    if (decoded != null) {
                        sb.append(decoded)
                        i += 2
                        continue
                ...
                // 四字节 GB18030 扩展（生僻字）或非法序列：跳过首字节
                sb.append('?')
                i += 1
            } else {
                i += 1
    ...
    /** 生成 Unicode→GBK 编码表：按 GB2312 区位反推 */
