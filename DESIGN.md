...
## LightNovelReader Integration Guardrails

> Upstream reference: [VoltAgent/awesome-design-md — Apple](https://github.com/voltagent/awesome-design-md/tree/main/design-md/apple), MIT License.

This file is a visual reference for the native SwiftUI iOS app, not a request to reproduce Apple's marketing website literally. Apply it under these project-specific rules:

- Apple Human Interface Guidelines, native iOS behavior, accessibility, Dynamic Type, safe areas, and existing reader functionality take precedence over the web-oriented rules below.
- Interpret pixel values as visual proportions rather than fixed iOS points. Prefer semantic SwiftUI fonts, colors, materials, controls, and system spacing.
- Do not introduce the website's black global navigation, marketing-page tiles, desktop breakpoints, or product-photography layout unless a user explicitly requests them.
- Preserve the app's selected theme, reading-background independence, pagination behavior, cover-loading performance, and iOS 16 compatibility.
- Use the reference mainly for restraint: clear hierarchy, generous whitespace, SF typography, one consistent accent, subtle materials, 44-point touch targets, and minimal decorative shadows.
- Never copy Apple trademarks, proprietary imagery, website text, or trade dress. Adapt general design principles to LightNovelReader's own identity.
