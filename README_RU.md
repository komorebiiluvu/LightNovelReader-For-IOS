**简体中文** | [繁體中文](README_TW.md) | [English](README_US.md) | [Русский](README_RU.md)

<div align="center">
    <img src="assets/logo.png" alt="logo" width="120"/>
    <h1>LightNovelReader (iOS)</h1>
    <a><img alt="iOS" src="https://img.shields.io/badge/iOS-000000?logo=apple&logoColor=white&style=for-the-badge"/></a>
    <a><img alt="Swift" src="https://img.shields.io/badge/Swift-F05138?logo=swift&logoColor=white&style=for-the-badge"/></a>
    <a><img alt="SwiftUI" src="https://img.shields.io/badge/SwiftUI-0D7CFF?logo=swift&logoColor=white&style=for-the-badge"/></a>
    <p>Читалка ранобэ для iOS на SwiftUI, поддерживает iPhone и iPad</p>
</div>

## Введение

LightNovelReader (iOS) — это открытая читалка ранобэ, написанная на Swift и SwiftUI, поддерживающая iPhone и iPad. Интерфейс и взаимодействие созданы по мотивам Android-версии [LightNovelReader](https://github.com/dmzz-yyhyy/LightNovelReader).

> ### ❤️ Особая благодарность
>
> **Этот проект глубоко вдохновлён и направлен проектом [LightNovelReader](https://github.com/dmzz-yyhyy/LightNovelReader). Без него этот проект не существовал бы.**
> Спасибо [dmzz-yyhyy](https://github.com/dmzz-yyhyy) и всем контрибьюторам за выдающуюся работу и открытый код.

## Возможности

- Реальный источник: просмотр, поиск и чтение онлайн
- Читалка: постраничный переворот, режим прокрутки, настройки шрифта / размера / интервала / фона, чисто-чёрный OLED-ночной режим
- Книжная полка: несколько полок, избранное и уведомления об обновлениях
- Офлайн-кэш: загрузка всей книги, чтение без сети
- Экспорт: EPUB / TXT
- Статистика чтения: учёт времени и тепловая карта

## Скриншоты

| | | |
|---|---|---|
| ![1](assets/screenshots/01.png) | ![2](assets/screenshots/02.png) | ![3](assets/screenshots/03.png) |
| ![4](assets/screenshots/04.png) | ![5](assets/screenshots/05.png) | ![6](assets/screenshots/06.png) |
| ![7](assets/screenshots/07.png) | | |

## Загрузка

Скачайте последний релиз с [GitHub Releases](https://github.com/komorebiiluvu/LightNovelReader-For-IOS/releases/latest). Релизы — это неподписанные `.ipa`-файлы: подпишите их самостоятельно перед установкой (например, с помощью Sideloadly, AltStore или аналогичных инструментов).

## Поддержка

- Сообщите об ошибке или предложите функцию через [Issues](https://github.com/komorebiiluvu/LightNovelReader-For-IOS/issues/new/choose)
- Свяжитесь с автором: **QQ: `3662909214`**

## Лицензия

```
Copyright (C) dmzz-yyhyy (夜鱼很业余) and contributors of LightNovelReader
Copyright (C) 2026 komorebiiluvu (iOS Port)

   Licensed under the Apache License, Version 2.0 (the "License");
   you may not use this file except in compliance with the License.
   You may obtain a copy of the License at

       http://www.apache.org/licenses/LICENSE-2.0

   Unless required by applicable law or agreed to in writing, software
   distributed under the License is distributed on an "AS IS" BASIS,
   WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
   See the License for the specific language governing permissions and
   limitations under the License.
```

Полный текст лицензии: см. [LICENSE](LICENSE). О встроенных шрифтах: см. [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).

## Отказ от ответственности

Это приложение — лишь личный инструмент для чтения. Оно не предоставляет и не хранит какой-либо контент; все права на контент принадлежат авторам и сайтам. Пожалуйста, поддерживайте оригинальные произведения.
