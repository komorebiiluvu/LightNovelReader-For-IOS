**简体中文** | [繁體中文](README_TW.md) | [English](README_US.md) | [Русский](README_RU.md)

<div align="center">
    <img src="assets/logo.png" alt="logo" width="120"/>
    <h1>LightNovelReader (iOS)</h1>
    <a><img alt="iOS" src="https://img.shields.io/badge/iOS-000000?logo=apple&logoColor=white&style=for-the-badge"/></a>
    <a><img alt="Swift" src="https://img.shields.io/badge/Swift-F05138?logo=swift&logoColor=white&style=for-the-badge"/></a>
    <a><img alt="SwiftUI" src="https://img.shields.io/badge/SwiftUI-0D7CFF?logo=swift&logoColor=white&style=for-the-badge"/></a>
    <p>A light novel reader built with SwiftUI for iOS, supports iPhone and iPad</p>
</div>

## Introduction

LightNovelReader (iOS) is an open-source light novel reader written in Swift/SwiftUI (UI) + Kotlin Multiplatform (data), supporting iPhone and iPad. Its UI and interactions are inspired by the Android app [LightNovelReader](https://github.com/dmzz-yyhyy/LightNovelReader).

> ### ❤️ Special Thanks
>
> **This project is deeply inspired and guided by [LightNovelReader](https://github.com/dmzz-yyhyy/LightNovelReader). Without it, this project would not exist.**
> Thanks to [dmzz-yyhyy](https://github.com/dmzz-yyhyy) and all contributors for their outstanding work and open-sourcing.

## Features

- Real book source: online browsing, searching and reading
- Reader: page-curl paging, scroll mode, adjustable font / size / spacing / background, OLED pure-black night mode
- Bookshelf: multiple shelves, favorites and update notifications
- Offline cache: full-book download, read without network
- Export: EPUB / TXT
- Reading stats: time tracking and heatmap

## Screenshots

| | | |
|---|---|---|
| ![1](assets/screenshots/01.png) | ![2](assets/screenshots/02.png) | ![3](assets/screenshots/03.png) |
| ![4](assets/screenshots/04.png) | ![5](assets/screenshots/05.png) | ![6](assets/screenshots/06.png) |
| ![7](assets/screenshots/07.png) | | |

## Download

Download the latest release from [GitHub Releases](https://github.com/komorebiiluvu/LightNovelReader-For-IOS/releases/latest). Release builds are unsigned `.ipa` files — sign them yourself before installing (e.g. with Sideloadly, AltStore, or similar tools).

## Support

- File a bug report or feature request at [Issues](https://github.com/komorebiiluvu/LightNovelReader-For-IOS/issues/new/choose)
- Contact the author: **QQ: `3662909214`**

## License

```
Copyright (C) dmzz-yyhyy (夜鱼很业余) and contributors of LightNovelReader
Copyright (C) 2026 komorebiiluvu (iOS Port)

   Licensed under the Apache License, Version 2.0 (the "License");
   you may not use this file except in compliance with the License.
   You may obtain a copy of the License at

       http://www.apache.org/licenses/LICENSE-2.0

   Unless required by applicable law or agreed to in writing, software
   distributed under the License is distributed on an "AS IS" BASIS,
   WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
   See the License for the specific language governing permissions and
   limitations under the License.
```

Full license text: see [LICENSE](LICENSE). Bundled fonts: see [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).

## Disclaimer

This app is a personal reading tool only. It does not provide or store any novel content;
all content belongs to its original authors and sites. Please support the original works.
